...
**`distance_decay` 实现了但默认关闭（2026-08-12 定案）。** 上面公式里的地理项
是**可选**的，`003_distance_decay.sql` 落地了 `geo_decay`（haversine + 指数半衰期），
`match_recommendations` / `pair_score` / `score_baseline` 三个 RPC 都接 `p_half_life_km`
参数，但 `MATCH_HALF_LIFE_KM` 默认不设 → 因子恒为 `1.0`，匹配纯语义。

关掉不是没做完，是量过之后的决定。在 138 人池上实测 5 对 ground-truth 配对：

```
配对                  关闭(现状)   3000km     10000km
跨 伦敦-东京           0.2767     -0.1748     0.0204
跨 伦敦-柏林           0.2378      0.3121     0.3262
跨 SF-东京             0.2087     -0.0927     0.0258
同城 SF                0.1923      0.4725     0.3208
同城 NYC               0.1563      0.4337     0.2557
```

伦敦-东京本是全池语义最强的一对（0.2767），开 3000km 后变成全池最差（-0.1748）
并跌破基线，排名完全反转；退到 10000km，两对跨太平洋配对仍是 0.02 上下，低于
§ 判定里的噪音上限 0.09，agent 会判成"没有匹配"。而 30000km 那种近乎无操作的值，
反而把最弱真配对从 0.1611 压到 0.1389——只有代价没有收益。

根因是**模型形状不对，不是常数选错**：距离重不重要取决于意图（合租房、线下服务
是刚需；投资人、语言交换、远程协作无关），全局乘子表达不了这个区别。真要让地理
生效，正确形状是按意图启用，或退化成"语义分接近时的次级排序"，都不是调一个常数
能解决的。此外开启会让已定稿的判定带 `(0.0756, 0.1492)` 立即失效，需连带重校准。

结论：保留实现、保持关闭。复现无需改 `.env`——三个 RPC 的 `p_half_life_km` 直接传值即可。
